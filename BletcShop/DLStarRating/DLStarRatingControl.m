...
- (void)setupViewWithStarNoEdgeInset:(UIImage *)defaultStar highlightStar:(UIImage *)highlightStar{
	self.clipsToBounds = YES;
	currentIdx = -1;
	star = [defaultStar retain];
	highlightedStar = [highlightStar retain];
    
	for (int i=0; i<numberOfStars; i++) {
		DLStarView *v = [[DLStarView alloc] initWithNoEdgeInsetBottom:self.star highlighted:self.highlightedStar position:i allowFractions:isFractionalRatingEnabled];
		[self addSubview:v];
		[v release];
	}
}

- (void)setupViewWithStar:(UIImage *)defaultStar highlightStar:(UIImage *)highlightStar{
	self.clipsToBounds = YES;
	currentIdx = -1;
	star = [defaultStar retain];
	highlightedStar = [highlightStar retain];
...
- (id)initWithFrame:(CGRect)frame andStars:(NSUInteger)_numberOfStars isFractional:(BOOL)isFract star:(UIImage *)defaultStar highlightStar:(UIImage *)highlightStar {
	self = [super initWithFrame:frame];
	if (self) {
        isFractionalRatingEnabled = isFract;
		numberOfStars = _numberOfStars;
        if (isFractionalRatingEnabled)
            numberOfStars *=kNumberOfFractions;
		[self setupViewWithStar:defaultStar highlightStar:highlightStar];
	}
	return self;
}


- (id)initWithFrameNoEdgeInset:(CGRect)frame andStars:(NSUInteger)_numberOfStars isFractional:(BOOL)isFract star:(UIImage *)defaultStar highlightStar:(UIImage *)highlightStar {
	self = [super initWithFrame:frame];
	if (self) {
        isFractionalRatingEnabled = isFract;
		numberOfStars = _numberOfStars;
        if (isFractionalRatingEnabled)
            numberOfStars *=kNumberOfFractions;
		[self setupViewWithStarNoEdgeInset:defaultStar highlightStar:highlightStar];
	}
	return self;
}
