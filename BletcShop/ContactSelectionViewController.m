...
@property (strong, nonatomic) LZDSearchBar *searchBar;
...
{
    if (_searchBar == nil) {
        _searchBar = [[LZDSearchBar alloc] initWithFrame: CGRectMake(0, 0, self.view.frame.size.width, 44)];
...
- (UIView *)footerView
{
    if (self.mulChoice && _footerView == nil) {
        ...
        [_doneButton setBackgroundColor:NavBackGroundColor];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([header_S isEqualToString:@"u"]) {
        [cell.imageView  sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",HEADIMAGE,p.imgStr]] placeholderImage:[UIImage imageNamed:@"chatListCellHead.png"]];
        
    }else{
        [cell.imageView  sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",SHOPIMAGE_ADDIMAGE,p.imgStr]] placeholderImage:[UIImage imageNamed:@"chatListCellHead.png"]];
...
//            [img sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",HEADIMAGE,p.imgStr]] placeholderImage:[UIImage imageNamed:@"user"]];
            
//            remarkView.image = img.image;
//            remarkView.image = [UIImage imageNamed:@"user"];

            NSString *header_S = [[p.idstring componentsSeparatedByString:@"_"] firstObject];
            
            if ([header_S isEqualToString:@"u"]) {
                remarkView.imgStr = [NSString stringWithFormat:@"%@%@",HEADIMAGE,p.imgStr];
                
            }else{
                remarkView.imgStr = [NSString stringWithFormat:@"%@%@",SHOPIMAGE_ADDIMAGE,p.imgStr];
                
            }

            
            remarkView.remark = p.name;
...
- (void)loadDataSource
{
    if (!_presetDataSource) {
        ...
        HUD.label.text = NSLocalizedString(@"loadData", @"Load data...");
