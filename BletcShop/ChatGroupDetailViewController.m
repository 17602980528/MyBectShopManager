...
@interface ChatGroupDetailViewController ()<EMGroupManagerDelegate, EMChooseViewDelegate, UIActionSheetDelegate,UIAlertViewDelegate>
        ...
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [self.dataSource count]) {
                ...
                if ([header_S isEqualToString:@"u"]) {
                    contactView.imgStr =[NSString stringWithFormat:@"%@%@",HEADIMAGE,p.imgStr];
                }else{
                    contactView.imgStr =[NSString stringWithFormat:@"%@%@",SHOPIMAGE_ADDIMAGE,p.imgStr];
