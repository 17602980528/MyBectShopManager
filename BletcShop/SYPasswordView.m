...
- (void)textFieldDidChange:(UITextField *)textField
{
    ...
    if (textField.text.length == kDotCount) {
        NSLog(@"输入完毕==%@",self.textField.text);
        if ([_delegate respondsToSelector:@selector(passLenghtEqualsToSix:)]) {
            [_delegate passLenghtEqualsToSix:self.textField.text];
        }
    }
    if ([_delegate respondsToSelector:@selector(observationPassLength:)]) {
        [_delegate observationPassLength:self.textField.text];
    }
}
