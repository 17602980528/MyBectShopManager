 ...
 *获取所有安卓默认表情
 */
+ (NSArray *)ObtainAllAndroidDefaultExpression{
    NSString *filePath = [self ObtainFilePath:@"android.emoji"];
...
+ (UIImage *)ObtainPictureDefault:(NSString *)name{
    ...
    NSString *filePath = [NSString stringWithFormat:@"%@/%@",[self ObtainExpressionPath:@"android.emoji"],name];
...
+ (UIImage *)ObtainPictureNameDefault:(NSString *)name{
    NSArray *sinaArry = [self ObtainAllAndroidDefaultExpression];
    for (int i=0; i<sinaArry.count; i++) {
        NSDictionary *dict = sinaArry[i];
        
        NSLog(@"%@--安卓-%@",dict[@"chs"],name);
